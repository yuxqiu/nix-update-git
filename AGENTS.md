...
- `src/cli.rs` — clap CLI definition (`--check`, `--update`, `--interactive`, `--verbose`, `--jobs`, `--rules`); no `--format` — text diff output is the only format
...
- `src/rules/flake_input.rs` — the main rule; `FlakeUrl::Forge { forge, owner, repo }` handles any registered forge with a `flake_scheme()` (`github:`, `gitlab:`, `sourcehut:` today) via a loop over `forge::FORGES` rather than one match arm per scheme; `GitRemote`/`GitLocal` (for `git+https/ssh/file://`) are unrelated to forges and unchanged. Detects version updates via `git ls-remote`. Note: `SourceHut::remote_url_for_flake` preserves a pre-existing quirk carried over from before this was a `Forge` impl — it always prepends `~` to the owner (so an owner parsed with one already, e.g. from `sourcehut:~user/repo`, would double up) and resolves against `sr.ht` rather than `git.sr.ht`; `display_for_flake` has neither issue. Not fixed as part of this refactor — worth a follow-up.
