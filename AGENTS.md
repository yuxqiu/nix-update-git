...
- `src/forge/` (lib) — registry of git hosting forges. `mod.rs` defines the `Forge` trait (`id`, `nixpkgs_fn_name`, `flake_scheme`, `extra_attrs`, `git_url`, `display_target`, `archive_url`, `remote_url_for_flake`, `display_for_flake`) plus `COMMON_GIT_ATTRS` (the attribute keys shared by every forge) and the `FORGES: &[&dyn Forge]` registry; `github.rs`/`gitlab.rs`/`gitea.rs`/`forgejo.rs`/`codeberg.rs`/`sourcehut.rs`/`bitbucket.rs`/`repoorcz.rs`/`gitiles.rs` each implement `Forge` in one small file. Adding a forge means adding one file + one line in `FORGES` — `kind.rs`, `tarball.rs`, and `flake_input.rs` all dispatch through the trait generically and need no changes. `fetchgit`, `builtins.fetchGit`, `fetchpatch`, and `fetchTarball` are deliberately not forges (url-keyed or structurally unrelated; see module doc).
