...
- `src/rules/mk_derivation.rs` — `MkDerivationRule` detects `stdenv.mkDerivation rec { version = "..."; src = fetchX { rev = "<hash>"; ... }; }` patterns. Only activates when `version` is a version string and `rev` is a 40-character commit hash. Updates `version` to the latest tag, `rev` to the corresponding SHA, and `hash`/`sha256` accordingly. No conflict with the fetcher rule because the fetcher rule already skips commit hashes.
