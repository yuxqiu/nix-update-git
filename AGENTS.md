...
- `src/rules/mk_derivation.rs` — `MkDerivationRule` detects `stdenv.mkDerivation rec { version = "..."; src = fetchX { rev = "<hash>"; ... }; }` patterns via `try_extract_call`. Only activates when `version` is a version string and `rev` is a 40-character commit hash. Updates `version` to the latest tag, `rev` to the corresponding SHA, and `hash`/`sha256` accordingly. No conflict with the fetcher rule because the fetcher rule already skips commit hashes.
...
- `MkDerivationRule` only activates when `version` is a version string AND `rev` is a 40-character commit hash. When `rev` is a version string (e.g. `rev = "v1.0.0"`), the fetcher rule handles it instead. There is no conflict because the fetcher rule skips commit hashes (which the mkDerivation rule targets).
